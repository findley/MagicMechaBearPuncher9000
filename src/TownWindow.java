...
public class TownWindow extends NodeWindow{
    ...
    public void render(GameContainer container, StateBasedGame game, Graphics g, Player[] player) throws SlickException {
        for(int i = 0; i < players.length; i++){
            if(inNode[i]){
                ...
                player[i].render(container, game, g, playerPos[i][0], playerPos[i][1]);
                GameState state = (GameState) (game.getCurrentState());
                g.setFont(state.uFont);
                g.setColor(Color.black);
                g.drawString(Double.toString(Math.ceil(timer)), players[i].windowPos[0] + container.getWidth()/4, players[i].windowPos[1]);
    ...
    public void init(GameContainer container, StateBasedGame game, Player[] players) throws SlickException {
        super.init(container, game, players);
            for(int i = 0; i < players.length; i++){
                playerPos[i][0] = players[i].windowPos[0] + players[i].windowSize[0] - players[i].pWidth;
                playerPos[i][1] = players[i].windowPos[1] + (int) players[i].windowSize[1] / 2;
                timer = 4000.0;
            }
    }
        ...
        for (int i = 0; i < players.length; i++){
            if(inNode[i]){
                ...
                if (input.isKeyDown(players[i].getButton("right"))) {
                    if (playerPos[i][0] + players[i].pWidth + moveValue < players[i].windowPos[0] + players[i].windowSize[0]) {
                        playerPos[i][0] += moveValue;
                    }
                }
                ...
                if (input.isKeyDown(players[i].getButton("down"))) {
                    if (playerPos[i][1] + players[i].pHeight + moveValue < players[i].windowPos[1] + players[i].windowSize[1]) {
                        playerPos[i][1] += moveValue;
                    }
                }
            }
