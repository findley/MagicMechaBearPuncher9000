...
public class Camera {
	...
	public void centerOn(float x, float y) {
		...
		if (cameraX <= 0)
			cameraX = 0;
		if (cameraX + player.windowSize[0] > mapWidth)
			cameraX = mapWidth - player.windowSize[0];
		...
		if (cameraY + player.windowSize[1] > mapHeight)
			cameraY = mapHeight - player.windowSize[1];
	}
	...
	public void drawMap(int offsetX, int offsetY) {
		...
		map.render(tileOffsetX + offsetX, tileOffsetY + offsetY, tileIndexX,
				tileIndexY, ((int) player.windowSize[0] - tileOffsetX)
						/ tileWidth + 1,
